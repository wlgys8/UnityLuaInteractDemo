...
#pragma warning disable IDE1006 // Naming Styles
        ...
        public static voidptr_t lua_newuserdata(lua_State luaState, size_t size){
        ...
        internal static int lua_pcall(lua_State luaState,
            int nargs,
            int nresults,
            int errorfunc){
                return lua_pcallk(luaState,nargs,nresults,errorfunc,System.IntPtr.Zero,System.IntPtr.Zero);
            }
        ...
        public static void lua_pushcfunction(lua_State luaState, LuaFunction f){
            lua_pushcclosure(luaState,f.ToFunctionPointer(),0);
        }
        ...
        public static void lua_pushbuffer(lua_State luaState,byte[] buffer)
        {
            if(buffer == null)
            {
            ...
            NativeMethods.lua_pushlstring(luaState, buffer, (size_t)buffer.Length);
        }

        public static void lua_pushstring(lua_State luaState,string s){
            if(s == null){
                lua_pushnil(luaState);
                return;
            }
            lua_pushbuffer(luaState,System.Text.Encoding.UTF8.GetBytes(s));
        }
        ...
        internal static void lua_pop(lua_State luaState,int n){
            NativeMethods.lua_settop(luaState, -n - 1);
        }
        ...
        internal static byte[] ToBuffer(lua_State luaState,int index, bool callMetamethod)
        {
            size_t len;
            System.IntPtr buff;
            if (callMetamethod)
        ...
        internal static string lua_tostring(lua_State luaState,int index){
            byte[] buffer = ToBuffer(luaState,index, false);
            if(buffer == null)
                return null;
        ...
        public static double lua_tonumber(lua_State luaState,int index)
        {
            int isNum;
        ...
        internal static extern int luaL_loadbufferx
            ...
            string name,
            string mode);
        internal static int luaL_loadbuffer(lua_State luaState,byte[] buffer, string name)
        {
            return NativeMethods.luaL_loadbufferx(luaState, buffer, (size_t)buffer.Length, name, null);
        }

        internal static int luaL_loadbuffer(lua_State luaState,string buffer, string name)
        {
            return NativeMethods.luaL_loadbufferx(luaState, System.Text.Encoding.UTF8.GetBytes(buffer), (size_t)buffer.Length, name, null);
        }
